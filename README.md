...
```sql
SELECT CustomerID, CompanyName, Region FROM Customers ORDER BY (CASE WHEN Region iS NULL THEN 1 ELSE 0 END) ASC, Region ASC, CustomerID ASC
```
...
```sql
SELECT TOP 3 ShipCountry, AverageFreight = AVG(Freight) FROM Orders WHERE CONVERT(date, OrderDate) LIKE '2015%' GROUP BY ShipCountry ORDER BY AverageFreight DESC
```
...
```sql
SELECT TOP 3 ShipCountry, AverageFreight = AVG(Freight) FROM Orders WHERE OrderDate > (SELECT DATEADD(YEAR, -1, MAX(OrderDate)) FROM Orders) GROUP BY ShipCountry ORDER BY AverageFreight DESC
```

## 29. Inventory list
We're doing inventory, and need to show information like the below, for all orders. Sort by OrderID and Product ID.
```sql
SELECT Employees.EmployeeID, LastName, Orders.OrderID, ProductName, Quantity FROM Orders JOIN OrderDetails ON Orders.OrderID = OrderDetails.OrderID JOIN Products ON OrderDetails.ProductID = Products.ProductID JOIN Employees ON Orders.EmployeeID = Employees.EmployeeID  ORDER BY Orders.OrderID, OrderDetails.ProductID
```